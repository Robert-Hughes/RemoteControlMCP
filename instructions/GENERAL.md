...
- For bounded foreground execution, provide both `timeout_ms` and `timeout_action`. With `detached = true`, `timeout_action = "detach"` is redundant, so the server ignores both timeout fields and launches detached normally. A detached `timeout_action = "stop"` remains meaningful and stops the process after `timeout_ms` if it is still running.
...
- `insert_before_line` and `insert_after_line` anchor non-empty text to an existing 1-based line only when `expected_anchor_text` matches that line's current unnumbered logical content. Copy the anchor content shown by `read_file` without the `<line_number>: ` prefix or line terminator; use an empty string for a blank anchor. Pass file content only in `text`. A mismatch is rejected without changing the file. Do not simulate insertion by replacing and reproducing an adjacent line.
